(function () {
  ...
  var LANG_TOTAL = { en: 103.76, ms: 131.92, ja: 157.28 };
  ...
  function langDuration(lang) {
    if (narrationEl && narrationEl.dataset.lang === lang &&
        narrationEl.duration && isFinite(narrationEl.duration)) {
      return narrationEl.duration;
    }
    return LANG_TOTAL[lang] || LANG_TOTAL.en;
  }

  function resetSyncClock(t) {
    syncBaseTime = t || 0;
  ...
  function currentSyncTime() {
    if (narrationEl && !narrationEl.paused && !narrationEl.ended) {
      return narrationEl.currentTime;
    }
    return syncBaseTime + (performance.now() - syncBasePerf) / 1000;
  }

  function advanceLangCycle() {
