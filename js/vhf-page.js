(function () {
  ...
  function calcNetworkSiteOnlyLimits(sites) {
    const xs = sites.map((s) => s.x_km);
    const ys = sites.map((s) => s.y_km);
    return calcNetworkPlanLimits([], [], sites);
  }
  ...
  function initSiteView() {
    ...
    const tickStep = chooseKmTickStep(netLim.span);
    ...
    const planLayout = {
      ...
      yaxis: {
        ...plotLayout.yaxis, title: 'South-North (km)',
        range: netLim.y, tickmode: 'array', tickvals: ticksY, ticktext: ticksY.map(String),
        scaleanchor: 'x', scaleratio: 1,
      },
      showlegend: false,
    };

    Plotly.react(planPlotId, [planSites], planLayout, { responsive: true, displayModeBar: false });
    ...
    const box3d = {
      ...
      x: boxLines.x, y: boxLines.y, z: boxLines.z,
      line: { color: 'rgba(96,165,250,0.55)', width: 2 },
      hoverinfo: 'skip',
    };
    ...
    Plotly.react(plot3dId, [sites3d, box3d], layout3d, {
    ...
    plot2dIds.slice(1).forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.innerHTML = '';
    });

    if (flashMeta) flashMeta.textContent = 'No lightning events available. Showing network layout.';
  }
  ...
  function startSiteRotation() {
    ...
    function tick() {
      ...
      const boxLines = rotateLineXY(anim3d.boxBase.lines.x, anim3d.boxBase.lines.y, anim3d.angle, cx, cy);
      Plotly.restyle(plot3dId, {
        x: [sites.x, boxLines.x],
        y: [sites.y, boxLines.y],
      }, [0, 1]);
      siteRotRafId = requestAnimationFrame(tick);
    }
    siteRotRafId = requestAnimationFrame(tick);
  }
  ...
  function bind3dInteraction() {
    ...
    if (!wrap || wrap.dataset.vhf3dBound) return;
    wrap.dataset.vhf3dBound = '1';
